...
import { loadWebFonts, registerWebFonts } from '@/core/font-loader';
import { CommandRegistry } from '@/command/registry';
import { CommandDispatcher } from '@/command/dispatcher';
import type { EditorContext, CommandServices } from '@/command/types';
import { fileCommands, saveDocument } from '@/command/commands/file';
...
import { showCloseConfirm } from '@/ui/close-confirm-dialog';
...
let totalSections = 1;
let closeDialogInProgress = false;

async function confirmClose(): Promise<boolean> {
  if (wasm.pageCount === 0) return true;

  const choice = await showCloseConfirm(wasm.fileName);
  if (choice === 'cancel') return false;
  if (choice === 'dont-save') return true;

  // save 선택
  if (wasm.getSourceFormat() === 'hwpx') {
    alert('HWPX 문서는 현재 저장이 비활성화되어 있습니다. 저장 없이 종료합니다.');
    return true;
  }

  return saveDocument(commandServices);
}

async function handleCloseRequest(source: 'renderer' | 'main'): Promise<void> {
  if (closeDialogInProgress) return;
  ...
  try {
    ...
    if (source === 'main' && window.hanpa?.sendAppCloseResponse) {
      window.hanpa.sendAppCloseResponse(true);
    } else {
      window.close();
    }
  } finally {
...
function setupWindowCloseHandlers(): void {
  window.addEventListener('beforeunload', (event) => {
    if (closeDialogInProgress) return;
    event.preventDefault();
    handleCloseRequest('renderer');
  });

  window.hanpa?.onAppCloseRequest(() => {
    handleCloseRequest('main');
  });
}
...
function parseVersion(version: string): { major: number; minor: number; patch: number; qualifier: string | null } | null {
  const normalized = normalizeVersion(version);
  const [main, qualifier] = normalized.split('-', 2);
...
function isNewerVersion(current: string, latest: string): boolean {
  const currentSemver = parseVersion(current);
  const latestSemver = parseVersion(latest);
  ...
  if (!currentSemver.qualifier && latestSemver.qualifier) {
    return false;
  }
  if (currentSemver.qualifier && !latestSemver.qualifier) {
    return true;
  }
  return latestSemver.qualifier !== currentSemver.qualifier && latestSemver.qualifier !== null;
}

async function checkForAppUpdate(): Promise<void> {
  const currentVersion = normalizeVersion(__APP_VERSION__);
  const storageKey = 'hanpa.lastNotifiedReleaseVersion';
  const lastNotifiedVersion = window.localStorage.getItem(storageKey);
  const releaseUrl = 'https://api.github.com/repos/Michael09011/HANPA/releases/latest';
  ...
  try {
    ...
    let latestVersion = String(release.tag_name ?? release.name ?? '').trim();
    if (!latestVersion) return;
    latestVersion = normalizeVersion(latestVersion);
    if (!isNewerVersion(currentVersion, latestVersion)) return;
    if (lastNotifiedVersion === latestVersion) return;

    showToast({
      message: `HANPA 새 버전 ${latestVersion}이 출시되었습니다.
지금 업데이트하려면 자세히를 눌러주세요.`,
      durationMs: 0,
      action: {
        label: '자세히',
        onClick: () => {
          window.open(release.html_url ?? 'https://github.com/Michael09011/HANPA/releases/latest', '_blank');
        },
      },
      confirmLabel: '닫기',
    });
    ...
    console.warn('[update] 최신 릴리스 확인 실패:', error);
  }
}
